...
import { Churp as IChurp, Comment } from '@/lib/types'
...
export const Churp: FC<IChurp> = ({
  ...
  const { deleteChurp, like, unlike, isLiking, isUnliking } = useBurdAccount()
  const [isDeletingThis, setIsDeletingThis] = useState<boolean>(false)
  const [churpComments, setChurpComments] = useState<Comment[]>(comments)

  const deleteThis = async (id: string) => {
    try {
      setIsDeletingThis(true)
      console.log(id)
      await deleteChurp(id)
      await fetch()
  ...
  const handleNewComment = (commentContent: string) => {
    const newComment = {
      id: churpComments.length + 1,
      username: 'hathbanger',
      displayName: 'hath',
      content: commentContent,
      profileImageUrl: profileImageUrl,
    }
    setChurpComments([...churpComments, newComment])
  }
